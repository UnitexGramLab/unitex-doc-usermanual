...
\index{GPL}\index{LGPL}\index{License!GPL}\index{License!LGPL}
...
(\cite{TRE}), which is under GPL licence (\cite{GPL}). The LGPL Licence is more
permissive than the GPL licence, because it makes it possible to use LGPL code in
nonfree software. From the point of view of the user, there is no difference,
...
\bigskip
\noindent Full text versions of GPL, LGPL and LGPLLR can be found in the
...
administrator permissions. Decompress the file \verb+Unitex2.0.zip+ in a
directory named \verb+Unitex+, by using the following command:

\bigskip \noindent \verb$unzip Unitex2.0.zip -d Unitex$
...
on the icon showing a file (see figure~\ref{fig-creation-dossier-personnel}).
...
\caption{Creating the personal work directory\label{fig-creation-dossier-personnel}}
\end{center}
\end{figure}
...
No matter which operating system you are working with, it is sufficient to delete the \verb+Unitex+
directory to completely delete all the program files. Under Windows you may have to delete the shortcut to \verb+Unitex.jar+ \index{File!\verb+Unitex.jar+} if you have created one on your desktop. The same has to be done on Linux, if you have created an alias.
