...
\label{chap-grammaires-avancées}
...
are represented by the characters \verb+L+, \verb+R+, \verb+C+ and \verb+D+. All letters that are not 
operators are characters. The only
...
(cf. figure~\ref{fig-chemins-équivalents-dans-grammaire-flexion}).\index{Respect!of
lowercase/uppercase}
...
\caption{Two equivalent paths in an inflection grammar\label{fig-chemins-équivalents-dans-grammaire-flexion}}
\end{center}
\end{figure}
...
\bigskip
\noindent For more details, see section \ref{section-flexion-automatique}.
...
The figures~\ref{fig-exemple-découpage-en-phrases} (page
\pageref{fig-exemple-découpage-en-phrases})
and~\ref{fig-grammaire-de-normalisation} (page
\pageref{fig-grammaire-de-normalisation}) show examples of preprocessing graphs.


\subsection{Graphs for normalizing the text automaton}
\label{section-graphes-normalisation-automate}
...
ambiguity explicit. Figure~\ref{fig-extrait-grammaire-normalisation} shows an
...
\caption{Extract of the normalization graph used for French\label{fig-extrait-grammaire-normalisation}}
\end{center}
\end{figure}
...
section~\ref{section-sous-graphes}). They also have outputs including outputs
...
(cf. section~\ref{section-modification-du-texte}).

\bigskip
\noindent Syntactic graphs can use contexts (see section \ref{contextes}).
...
described in section \ref{grammaires-elag}, page \pageref{grammaires-elag}.
...
graphs are explained in chapter~\ref{chap-lexique-grammaire}.

\section{Compilation of a grammar}
\label{section-compiler-une-grammaire}
...
window (cf. Figure~\ref{fig-fenêtre-de-compilation}).

\bigskip
\begin{figure}[!h]
\begin{center}
\includegraphics[width=14.7cm]{resources/img/fig6-4.png}
\caption{Compilation window\label{fig-fenêtre-de-compilation}}
\end{center}
\end{figure}
...
window of Figure~\ref{fig-configuration-approximation} allows you to configure the
...
\caption{Configuration of approximation of a grammar\label{fig-configuration-approximation}}
\end{center}
\end{figure}
...
is equivalent to the original grammar (cf. Figure~\ref{fig-résultat-approximation}).

\bigskip
\begin{figure}[!h]
\begin{center}
\includegraphics[width=14.7cm]{resources/img/fig6-6.png}
\caption{Resultat of the approximation of a grammar\label{fig-résultat-approximation}}
\end{center}
\end{figure}
...
output (cf. Figure~\ref{fig-transduction-sous-graphe}).
...
\caption{How to associate an output with a call to a subgraph\label{fig-transduction-sous-graphe}}
\end{center}
\end{figure}
...
Figure~\ref{fig-boucle-infinie-mot-vide-avec-transduction}.
...
Figure~\ref{fig-boucle-infinie-mot-vide-avec-transduction} will not be suppressed
...
transduction\label{fig-boucle-infinie-mot-vide-avec-transduction}}
\end{center}
\end{figure}
...
Figure~\ref{fig-boucle-infinie-mot-vide-sous-graphe}: if the subgraph
...
\label{fig-boucle-infinie-mot-vide-sous-graphe}}
\end{center}
\end{figure}
...
\caption{Void loop caused by two graphs calling each other\label{fig-boucle-infinie-appels-récursifs}}
\end{center}
\end{figure}
...
figure~\ref{fig-boucle-infinie-appels-récursifs}. Each of these graphs can call
...
displayed in the compilation window. Figure~\ref{fig-message-erreur} shows the
message that appears if one tries to compile the graph \verb+Det+ of
Figure~\ref{fig-boucle-infinie-appels-récursifs}.

\begin{figure}[!h]
\begin{center}
\includegraphics[width=15cm]{resources/img/fig6-11.png}
\caption{Error message when trying to compile \texttt{Det}\label{fig-message-erreur}}
\end{center}
\end{figure}
...
\section{Contexts}
\index{Contexts}\label{contextes}
...
\caption{Using a right context\label{context1}}
\end{center}
\end{figure}

\bigskip
\noindent Figure~\ref{context1} shows a simple right context. The graph matches
numbers followed by a currency symbol, but this symbol will not appear in
...
using \verb+$![+ to indicate the right context start. Figure~\ref{context2}
...
\caption{Using a negative right context\label{context2}}
\end{center}
\end{figure}

\bigskip
\noindent Right contexts can appear anywhere in the graph, including the
beginning of the graph. Figure~\ref{context3} shows a graph that matches an adjective in the
right context of something that is not a past participle. In other words, this
...
\caption{Matching an adjective that is not ambiguous with a past participle\label{context3}}
\end{center}
\end{figure}

\bigskip
\noindent This mechanism allows you to formulate complex patterns. For
instance, the graph of figure~\ref{context4} matches a sequence of two 
...
\caption{Advanced use of right contexts\label{context4}}
\end{center}
\end{figure}

\bigskip
\noindent You can use nested contexts. For instance, the graph shown in
figure~\ref{context5} matches a number that is not followed by a dot, except for
a dot followed by a number. Thus, in the sequence \textit{5.0+7.=12}, this graph
...
\caption{Nested contexts\label{context5}}
\end{center}
\end{figure}
...
right context (cf. figure~\ref{context6}). If you apply this graph in MERGE
...
\caption{Variable defined inside a right context\label{context6}}
\end{center}
\end{figure}
...
like the one shown on Figure \ref{left-context1}. However, with such a grammar,
the context part on the left will be included in the match, as shown on Figure
\ref{left-context2}.
...
determiner\label{left-context1}}
\end{center}
\end{figure}
...
\ref{left-context1}\label{left-context2}}
\end{center}
\end{figure}
...
\ref{left-context3}. The effect of such a context is to use this part of the
grammar for computing matches, but to ignore it in the results, as shown on
Figure \ref{left-context4}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=9cm]{resources/img/fig6-17c.png}
\caption{Matching a noun after a left context\label{left-context3}}
\end{center}
\end{figure}
...
\caption{Results of the application of the grammar shown on Figure
\ref{left-context3}\label{left-context4}}
\end{center}
\end{figure}

\clearpage
\noindent All the outputs produced in the left context are ignored, as you can
see in the concordance of Figure \ref{left-context6}, showing the results
obtained with the grammar of Figure \ref{left-context5}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=9cm]{resources/img/fig6-17e.png}
\caption{Ignored output in a left context\label{left-context5}}
\end{center}
\end{figure}
...
\caption{Results of the application of the grammar shown on Figure
\ref{left-context5}\label{left-context6}}
\end{center}
\end{figure}

\bigskip
\noindent However, you can catch things with variables (see section
\ref{variables}) and use them outside the left context, as shown on grammar of
Figure \ref{left-context7}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=10cm]{resources/img/fig6-17g.png}
\caption{Using a variable in a left context\label{left-context7}}
\end{center}
\end{figure}
...
\caption{Results of the application of the grammar shown on Figure
\ref{left-context7}\label{left-context8}}
\end{center}
\end{figure}
...
results. For instance, the grammar shown on Figure \ref{left-context9} looks
for expressions like \verb$the animal's$, but only extract nouns, as you can
see on Figure \ref{left-context10}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=10cm]{resources/img/fig6-17i.png}
\caption{A grammar with both left and right contexts\label{left-context9}}
\end{center}
\end{figure}
...
\caption{Results of the application of the grammar shown on Figure
\ref{left-context9}\label{left-context10}}
\end{center}
\end{figure}
...
filters (see section \ref{filters}), as shown on Figure \ref{morpho1}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=8cm]{resources/img/fig6-17k.png}
\caption{Matching morphological things\label{morpho1}}
\end{center}
\end{figure}
...
\caption{Example of morphological zone in a grammar\label{morpho2}}
\end{center}
\end{figure}
...
between double quotes).
   ...
   \item You can use outputs.
    ...
    \item The meta \verb+#+, \verb+<PRE>+, \verb+<NB>+, \verb+<TOKEN>+,
...
adjective with the grammar shown on Figure \ref{morpho3}.
...
\caption{Configuration of morphological dictionaries\label{morpho4}}
\end{center}
\end{figure}
...
\ref{morpho4}. You can select as many dictionaries as you want, but they MUST
be \verb+.bin+ ones. Once done, you can apply your grammar and get results.
...
\verb+$xxx.CODE$+, as shown on Figure \ref{morpho5}. Moreover, such variables
can be used even after the end of the morphological mode, as shown on Figure
\ref{morpho7}.
 
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=16cm]{resources/img/fig6-17o.png}
\caption{Using a morphological variable\label{morpho5}}
\end{center}
\end{figure}
...
\caption{Results of grammar of Figure \ref{morpho5} applied in MERGE mode
\label{morpho6}}
\end{center}
\end{figure}
...
\caption{Using a morphological variable in normal mode\label{morpho7}}
\end{center}
\end{figure}
...
figure~\ref{explore-graph-paths} appears.


\begin{figure}[!h]
\begin{center}
\includegraphics[width=10.4cm]{resources/img/fig6-18.png}
\caption{Exploring the paths of a grammar\label{explore-graph-paths}}
\end{center}
\end{figure}
...
Figure~\ref{glace} with default settings (ignoring outputs, limit = 100 paths):
...
\caption{Sample graph \label{glace}}
\end{center}
\end{figure}
...
figure~\ref{build-graph-collection}.

\begin{figure}[!h]
\begin{center}
\includegraphics[width=9cm]{resources/img/fig6-20.png}
\caption{Building a graph collection\label{build-graph-collection}}
\end{center}
\end{figure}
...
sub-directories, as one can see in figure~\ref{graph-collection}, which shows the
output graph generated for our example.
...
\caption{Main graph of a graph collection\label{graph-collection}}
\end{center}
\end{figure}
...
\label{applying-transducers-rules}
...
\caption{Example of a transducer\label{fig-exemple-de-transducteur}}
\end{center}
\end{figure}

\bigskip
\noindent Look at the transducer in Figure~\ref{fig-exemple-de-transducteur}.
...
figure~\ref{fig-exemple-de-transducteur}}
\end{center}
\end{figure}
...
occurrences are presented (cf. Figure~\ref{fig-chevauchement-occurrences}).

\begin{figure}[!h]
\begin{center}
\includegraphics[width=13cm]{resources/img/fig6-24.png}
\caption{Overlapping occurrences in concordance\label{fig-chevauchement-occurrences}}
\end{center}
\end{figure}
...
graph (cf. section~\ref{section-modification-du-texte}).
...
As we have seen in section~\ref{section-utilisation-des-variables}, it is
...
surrounded by the character \verb-$- (cf. Figure~\ref{fig-définition-variable}).
...
Figure~\ref{fig-définition-variable}.

\begin{figure}[!p]
\begin{center}
\includegraphics[width=12cm]{resources/img/fig6-25.png}
\caption{Definition of a variable in a subgraph\label{fig-définition-variable}}
\end{center}
\end{figure}
...
Figure~\ref{fig-inversion-de-mots} in REPLACE mode to the text \textit{Ivanhoe}
results in the concordance of Figure~\ref{fig-pas-de-problème-espacement}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=11.3cm]{resources/img/fig6-27.png}
\caption{Inversion of words using two variables\label{fig-inversion-de-mots}}
\end{center}
\end{figure}
...
\caption{Result of the application of the transducer in figure~\ref{fig-inversion-de-mots}\label{fig-pas-de-problème-espacement}}
\end{center}
\end{figure}

\bigskip
\noindent If the beginning or the end of variable is malformed (end of a variable before
its beginning or absence of the beginning or end of a variable), it will be
ignored during the emission of outputs.
...
figure~\ref{fig-chevauchement-de-variables}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=15cm]{resources/img/fig6-29.png}
\caption{Overlapping variables\label{fig-chevauchement-de-variables}}
\end{center}
\end{figure}

\section{Applying graphs to texts}
\label{section-application-des-graphes-au-texte}
...
search in the window shown in figure~\ref{fig-recherche-expressions}.
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=8.8cm]{resources/img/fig6-30.png}
\caption{Locate pattern Window\label{fig-recherche-expressions}}
\end{center}
\end{figure}
...
The window of Figure~\ref{fig-configuration-affichage-occurrences} lets you
...
section~\ref{section-affichage-des-résultats}.

\bigskip
\begin{figure}[h]
\begin{center}
\includegraphics[width=11cm]{resources/img/fig6-31.png}
\caption{Configuration for displaying the encountered occurrences
\label{fig-configuration-affichage-occurrences}}
\end{center}
\end{figure}
...
(cf. section~\ref{section-affichage-des-résultats}).\index{Web browser}
...
Figure~\ref{fig-sélection-occurrence-dans-le-texte}).

\bigskip
\begin{figure}[h]
\begin{center}
\includegraphics[width=13.5cm]{resources/img/fig6-32.png}
\caption{Selection of an occurrence in the text\label{fig-sélection-occurrence-dans-le-texte}}
\end{center}
\end{figure}
...
\label{section-modification-du-texte}
...
Figure~\ref{fig-configuration-affichage-occurrences}. This file has to have the
extension \verb+.txt+.
\index{File!\verb+.txt+}
...
section~\ref{applying-transducers-rules}.
...
\ref{fig-configuration-affichage-occurrences}). Then, click on "Extract matching
