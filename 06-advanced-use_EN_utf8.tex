...
between double quotes).
   ...
   \item You can use outputs.
    ...
    file (<MOT> is deprecated).\index{\verb+<MOT>+}

    \item \verb+<LOWER>+ will match any lowercase letter, as defined in the
    alphabet file (<MIN> is deprecated).\index{\verb+<MIN>+}\index{\verb+<LOWER>+}

    \item \verb+<UPPER>+ will match any uppercase letter, as defined in the
    alphabet file  (<MAJ> is deprecated).\index{\verb+<MAJ>+}\index{\verb+<UPPER>+}

    \item \verb+<DIC>+ will match any word present in a morphological-mode
    dictionary\index{\verb+<DIC>+}, but the meta-symbols \verb+#+, \verb+<PRE>+, \verb+<FIRST>+, \verb+<NB>+, 
    \verb+<SDIC>+ and \verb+<CDIC>+ are forbidden.\index{\verb+#+} \index{\verb+<PRE>+}\index{\verb+<NB>+} \index{\verb+<TOKEN>+}  \index{\verb+<SDIC>+} \index{\verb+<CDIC>+}\index{\verb+<FIRST>+}
