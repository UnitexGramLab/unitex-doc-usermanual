...
In a regular expression, a token is defined as in \ref{decoupage-du-texte} (page
\pageref{decoupage-du-texte}). Note that the symbols dot, plus, star, less than,
opening and closing parentheses and double quotes have a
...
\label{section-symboles-spéciaux}
\index{Meta-symbols}

There are two kinds of lexical masks. The first category contains all symbols that have
been introduced in section~\ref{section-découpage-en-phrases} except for the
...
\bigskip
\noindent NOTE: as described in section \ref{decoupage-du-texte}, NO meta can be used to match the
\verb+{STOP}+\index{\verb+{STOP}+} marker, not even \verb+<TOKEN>+.
...
\verb+z3+ (cf. table~\ref{tab-codes-sémantiques}).
...
prohibited codes. \verb+<-N>+ and \verb+<-A-z1>+ are thus incorrect masks. However, you can express
such constraints using contexts (see section~\ref{contextes}).
...
\begin{itemize}
  ...
  verb \textit{suivre} (as opposed to the form of the verb \textit{être});
  ...
  \item \verb$<!MOT>$ : all tokens that are not made of letters (cf. figure~\ref{fig-recherche-<!MOT>}).
  This mask does not recognize the sentence separator \verb+{S}+ and the special tag \verb+{STOP}+.
...
\caption{Result of a search for the pattern \texttt{<!MOT>}\label{fig-recherche-<!MOT>}}
\end{center}
\end{figure}
...
figure~\ref{fig-erreur-expression-reconnaissant-epsilon}.

\bigskip
\begin{figure}[h]
\begin{center}
\includegraphics[width=14cm]{resources/img/fig4-3.png}
\caption{Error message when searching for the empty string\label{fig-erreur-expression-reconnaissant-epsilon}}
\end{center}
\end{figure}
...
\begin{itemize}
  ...
  \item \verb$<<[aeiouy]{3,5}>>$: contains a sequence of non-accentuated vowels whose length is between 3 and 5
  \item \verb$<<ée?>>$: contains \verb$é$ followed by an optional \verb$e$
...
\begin{itemize}
  ...
  \item \verb+<<^([rst][aeiouy]){2,}$>>+: a word formed by 2 or more sequences beginning with \verb$r$, \verb$s$
  ...
  are better described using contexts (see section~\ref{contextes}).
\end{itemize}
...
\verb$<<^é>>$ will recognize all the words starting with \verb$é$, but also those
which start with \texttt{\'E} or \verb$E$. To force the matcher to respect case, 
add \verb$_f_$ immediately after the filter, \textit{e.g.}:
\verb$<A><<^é>>_f_$.
...
chapter~\ref{chap-texte}). Then click on "Locate Pattern..." in the "Text" menu.
The window of figure~\ref{fig-configuration-recherche-expression-rationnelle}
appears.
...
\caption{``Locate pattern'' window\label{fig-configuration-recherche-expression-rationnelle}}
\end{center}
\end{figure}
...
expressions. They are described in  section~\ref{section-application-des-graphes-au-texte}.
...
\label{section-affichage-des-résultats}
When the search is finished, the window of figure~\ref{fig-résultat-recherche}
appears showing the number of matched occurrences, the number of recognized
tokens and the ratio between this number and the total number of
tokens in the text.
...
\caption{Search results \label{fig-résultat-recherche}}
\end{center}
\end{figure}
...
\caption{Configuration of the presentation of the found occurrences\label{fig-configuration-concordance}}
\end{center}
\end{figure}
...
chapter~\ref{chap-grammaires-avancées}.
...
(cf. figure~\ref{fig-sélection-navigateur-web}).

\bigskip
\noindent \index{Concordance frame} If you choose to open the concordance in
Unitex, you will see a window as shown on Figure \ref{exemple-concordance}. 
...
\caption{Selection of a web browser for displaying concordances\label{fig-sélection-navigateur-web}}
\end{center}
\end{figure}
...
\caption{Example concordance\label{exemple-concordance}}
\end{center}
\end{figure}
