...
\item Le \textit{tableau} de droite affiche la cascade: la liste ordonnée des transducteurs et les options sélectionnées pour chaque graphe.
	...
	\item \textbf{iter}: Si le transducteur doit être appliqué une fois ou plusieurs fois
			jusqu'à ce que le texte soit inchangé (voir \ref{sub:AppWhiCon}).
	\end{itemize}
...
fournie à l'aide d'un fichier liste de transducteurs.
...
Ce comportement est sélectionné ou non pour chaque graphe selon que la case \verb+iter+ est cochée ou non.\\
...
Considérons le texte \emph{B B B A A A}. L'application du graphe \ref{fig:AB->A} sur ce texte avec {\it iter}  donne :\\
...
Le résultat de l'application d'une cascade est un fichier d'index (concord.ind), comme c'est le cas
lors d'une recherche de motif avec "Locate". Ce fichier d'index contient toutes les séquences
...
La sortie texte XML-isée est copiée dans le fichier \verb+exemple_csc.txt+ et le texte brut dans le fichier  \verb+exemple_raw.txt+
...
La DTD correspondant à notre format est la suivante :
