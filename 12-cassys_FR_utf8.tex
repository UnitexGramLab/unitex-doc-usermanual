...
CasSys reconna\^{i}t un graphe g\'{e}n\'{e}rique par son nom qui doit commencer par un arrobas @.

\subsection{Structure d'un graphe g\'{e}n\'{e}rique}
Un chemin dans un graphe g\'{e}n\'{e}rique doit commencer par une bo\^{i}te avec un arrobas et, en sortie, une accolade ouvrante. C'est cette bo\^{i}te qui sera remplie par CasSys. La deuxi\`{e}me bo\^{i}te comprend en sortie l'\'{e}l\'{e}ment \`{a} chercher. Sur la figure \ref{fig12-3-01}, CasSys place dans la bo\^{i}te toutes les entr\'{e}es de cat\'{e}gorie \textit{x} extraites du dictionnaire du texte. Par exemple, CasSys extrait \textit{A} de la ligne \emph{\{A,.x\}} du dictionnaire du texte comme dans la figure \ref{fig12-3-01a} .
\begin{figure}[!htb]
  \centering
