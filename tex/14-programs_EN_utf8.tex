...
\index{External programs!\verbc{CasSys}}\index{\verbc{CasSys}}\index{Cascade of transducers}
...
\begin{itemize}
  ...
  \item \verb+-r X+/\verb+--transducer_dir=X+: take transducer on directory X (avoids specifying full path for each transducer); X must be (back)slash terminated
\item \verb+-w DIC/--morpho=DIC+: specifies that \verb+DIC+ is a \verb+.bin+ dictionary for the morphological mode.
		Use one  \verb+-w XXX+ option for each \verb+.bin+ file, or delimit several \verb+.bin+ filenames with the colon character (:).
...
%//// -f FILE/--file=FILE: the snt text file
  \item \verb+-t TXT+/\verb+--text=TXT+: the text file to be modified, with extension .snt;
	\item \verb+-i+/\verb+--in_place+: means the same csc/snt directories must be used for each transducer
...
\item \verb+--standoff=+: specifies the name and path of the file for the standoff template and launches the creation of the standOff file for the text;
...
\bigskip
\noindent \textbf{Other Usage:} \verb+DumpOffsets [-M/--convert_common_to_modified] [OPTIONS] <txt>+
...
\section{UnitexTool}\index{External programs!\verbc{UnitexTool}}
\index{\verbc{UnitexTool}}\index{Scripting Unitex programs}
...
external programs. With it, you can chain programs so that they will be invoked
within a same system process, in order to speed up processing (see also
Chapter~\ref{chap-scripts}). This can be done by
invoking commands nested in curly brackets as follows:

\bigskip\index{External programs!\verbc{SelectOutput}}
\begin{verbatim}
...
\section{UnitexToolLogger}\index{External programs!\verbc{UnitexToolLogger}}
\index{\verbc{UnitexToolLogger}}\index{Log Unitex programs}
