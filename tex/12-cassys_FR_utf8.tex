...
un corpus balisé dans le style XML, et de produire un fichier résultat répertoriant les passages du
corpus qui sont délimités par des balises, avec le nombre d'occurrences de chaque passage distinct.
...
c'est-à-dire passages délimités par une balise ouvrante et la balise fermante correspondante).
Ce graphe aura généralement un contexte droit négatif (section~\ref{section-contexts}). 
Par exemple, le graphe de la figure \ref{fig-placeNameStandoff} permet d'inventorier les passages
balisés entre \verb$<placeName>$ et  \verb$</placeName>$.
...
\noindent Le programme peut traiter le cas où un élément contient un autre élément d'un nom différent~\footnote{Le nom d'un élément XML est le nom qui figure à l'intérieur de la balise.}, mais pas celui où il contient un élément du même nom. 

\bigskip
\noindent Pour utiliser l'option \verb$standoff$, on lance une cascade contenant le graphe. Le résultat est un fichier dont le nom est suffixé par \verb$_standoff$, et
qui liste les passages trouvés. Il les présente classés suivant les noms d'éléments,
et secondairement suivant les valeurs de l'attribut \verb$type$, si les balises ouvrantes possèdent
cet attribut. Donnons un exemple avec le texte balisé suivant:
...
l'attribut \verb$type$. Par exemple, le graphe de la figure \ref{fig-placeNameHorsRegionStandoff} permet de répertorier les balises \verb$placeName$, sauf celles de type \verb$Region$.
...
\noindent En passant une cascade avec le graphe de la figure
\ref{fig-placeNameHorsRegionStandoff}, on obtient un résultat avec un seul paragraphe:
\begin{verbatim}
Voici les balises trouvées :
     Liste pour l'élément "placeName" et l'attribut "City" 
          terme="Tours" 
...
\item une ligne contenant juste \verb$#LINE$: la partie entre \verb$#LINE$ et \verb$#REST$ servira de modèle pour former dans le fichier résultat la partie sur un élément (ou sur une combinaison élément-type)
\item un texte qui introduit un nom d'élément, noté \verb${TYPE}$, et éventuellement une valeur de l'attribut \verb$type$, notée \verb${SUBTYPE}$. La partie optionnelle doit être délimitée entre doubles chevrons: \verb$<<...>>$
\item une ligne \verb$#BLOCK$: la partie entre \verb$#BLOCK$ et \verb$#END$ servira de modèle pour former dans le fichier résultat la partie sur un passage balisé
\item un texte introduisant un passage balisé, noté \verb${TERM}$
\item un texte introduisant le nombre d'occurrences de ce passage, noté \verb${COUNT}$
