...
\verb+Le chien mange la balle+. The corresponding text automaton after linearization is 
shown Figure \ref{fig7-linearize3}.
...
The TrainingTagger program produces two variants of the tagger. The first one could prune transitions only on grammatical codes 
(for example, \verb+under.A+ versus \verb+under.ADV+). The second one could prune transitions on grammatical and inflectional codes 
(for example, \verb+the.DET:s+ versus \verb+the.DET:p+). In order to use the tagger on a text automaton, one should pay attention 
on tagset and words. The tagset must be identical in both model and text automaton. For example, if the statistical model has been 
computed with the tag \verb+DET+ for the word \verb+the+, the corresponding tag in the text automaton must be \verb+DET+. If you apply dictionaries to
construct the text automaton, be sure that the tagset of dictionaries are identical to that of the model.
Forms of words must be identical too in both model and text automaton. For example, if the statistical model has been computed with the word
\verb+doesn't+, the corresponding word in the text automaton must be \verb+doesn't+. If you apply replacing or normalization graphs, 
words could undergo some form changements. In the case of \verb+doesn't+, it would become \verb+does not+.
...
shown on Figure \ref{fig7-linearize2} with "cat" tagger data. Linearization of the automaton with "morph" tagger data is shown Figure 
\ref{fig7-linearize4}.
...
you need to pay attention on tagset and words. Before computing a statistical model, you have to think about dictionaries and normalization
graphs that you will use to construct the text automaton. And then, you will have to do modifications on the annotated corpus 
if word forms or tagset do not match completely. For example, if the normalization graph transforms the word \verb+jusqu'+ into \verb+jusque+,
 the corresponding word into the annotated corpus must be \verb+jusque+. A French tagger is distributed with Unitex.
