...
removal with ELAG (\cite{ELAG}). It is not possible at the moment to search the
text automaton for patterns.
...
Figure~\ref{fig-exemple-automate-de-phrase} shows the automaton of the fourth
sentence of the text \textit{Ivanhoe}.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=15.5cm]{resources/img/fig7-1.png}
\caption{Sentence automaton example\label{fig-exemple-automate-de-phrase}}
\end{center}
\end{figure}

\bigskip
\noindent You can see in Figure~\ref{fig-exemple-automate-de-phrase}
...
(cf. section~\ref{section-affichage-des-automates-de-phrase}).
...
figure~\ref{fig-exemple-automate-de-phrase}). This choice allows you to
...
section~\ref{section-graphes-normalisation-automate}.
\bigskip
...
automaton. Figure~\ref{fig-exemple-graphe-normalisation-automate}
shows the part of the grammar used for the ambiguity of the
sequence \verb+l'+ in French.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=8.6cm]{resources/img/fig7-4.png}
\caption{Normalization of the sequence \texttt{l'}\label{fig-exemple-graphe-normalisation-automate}}
\end{center}
\end{figure}
...
figure~\ref{fig-résultat-normalisation-automate} is obtained.
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=10.2cm]{resources/img/fig7-5.png}
\caption{Automaton that has been normalized with the grammar of figure~\ref{fig-exemple-graphe-normalisation-automate}\label{fig-résultat-normalisation-automate}}
\end{center}
\end{figure}
...
\ref{section-conservation-des-meilleurs-chemins}). The normalization at the time of the 
construction of the automaton allows you to 
...
\label{section-normalisation-clitiques-portugais}
...
of the verb. For example, the sequence \textit{dir-me-ão} (\textit{they will tell
me}), corresponds to the complete verbal form \textit{dirão}, associated with 
the pronoun \textit{me}. In order to be able to manipulate this rewritten form,
...
figures~\ref{fig-1285-pas-normalise} and~\ref{fig-1285-normalise}  show the
automaton of a sentence after normalization of the clitics.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=11cm]{resources/img/fig7-6.png}
\caption{Non-normalized phrase automaton\label{fig-1285-pas-normalise}}
\end{center}
\end{figure}
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=11cm]{resources/img/fig7-7.png}
\caption{Normalized phrase automaton\label{fig-1285-normalise}}
\end{center}
\end{figure}
\clearpage
...
(cf. figure~\ref{fig-configuration-construction-automate}). This option
...
\label{section-conservation-des-meilleurs-chemins}
...
figure~\ref{fig-ambiguïté-mot-inconnu}, it can be seen that the adverb
...
word\label{fig-ambiguïté-mot-inconnu}}
\end{center}
\end{figure}
...
\caption{Automaton of a thaï sentence\label{fig-automate-phrase-thaï}}
\end{center}
\end{figure}

\bigskip
\noindent This phenomenon can also take place in the treatment of certain Asian languages
like Thaï. When words are not delimited, there is no other solution than to
...
Figure~\ref{fig-automate-phrase-thaï} shows an example of such an automaton of a
Thaï sentence.
\bigskip
...
(cf. figure~\ref{fig-configuration-construction-automate}).
...
\caption{Configuration of the construction of the text automaton \label{fig-configuration-construction-automate}}
\end{center}
\end{figure}
...
Figure~\ref{fig-automate-phrase-thaï-nettoyé} shows the automaton of
figure~\ref{fig-automate-phrase-thaï} after cleaning.
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=10cm]{resources/img/fig7-11.png}
\caption{Automaton of figure~\ref{fig-automate-phrase-thaï} after cleaning\label{fig-automate-phrase-thaï-nettoyé}}
\end{center}\end{figure}
...
\subsection{Grammars For Resolving Ambiguities}\label{grammaires-elag}
...
\caption{ELAG grammar \texttt{elag-tu.grf}\label{elag-tu}}
\end{center}
\end{figure}

\bigskip
\noindent Figure~\ref{elag-tu} shows an example of a grammar. The \textit{if}
part recognizes a verb in the 2$^{nd}$ person singular followed by a dash and
...
a pronoun. Figure~\ref{application-grammaire-tu} shows the result of the application 
of this grammar on the sentence "\textit{Feras-tu cela
bientôt$~$?}". One can see in the automaton at the bottom that the path
corresponding to \verb+tu+ past participle was eliminated.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=14cm]{resources/img/fig7-13.png}
\caption{Result of applying the grammar in figure~\ref{elag-tu} \label{application-grammaire-tu}}
\end{center}
\end{figure}
...
\caption{Use of the synchronization point\label{point-de-synchronisation}}
\end{center}
\end{figure}
...
case for example in figure~\ref{point-de-synchronisation}. This grammar is
...
figure~\ref{est-il} beginning with \textit{Est-il}, one can see 
that all non-verb interpretations of \verb+Est+ were removed.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=14cm]{resources/img/fig7-15.png}
\caption{Result of the application of the grammar in figure~\ref{point-de-synchronisation}\label{est-il}}
\end{center}
\end{figure}
...
to be used (see figure~\ref{fig:text-auto1}). The default file is \verb+elag.rul+. This
will launch the \verb+Elag+ program \index{External programs!\verb+Elag+} which
...
\subsection{Description of the tag sets}\label{tagset.def}
\index{ELAG tag sets}
...
\verb$NAME$, followed by an identifier (\verb$français$, for example). This is
followed by the \verb$POS$ sections for each part of speech. Each section
...
optional:
\begin{itemize}
  \item \verb$inflex$\index{\verb$inflex$}: this part enumerates the inflectional
...
sentence: \textit{J'ai reçu des coups de fil de ma mère hallucinants.}}. This
grammar will preserve the correct analysis of sentences like: \textit{Les
...
\index{External programs!\verb$ElagComp$} consists in building an automaton whose language is the
set of the sequences of lexical tags (or lexical analyses of a sentence) which
...
\caption{Optimized ELAG grammar checking verb-pronoun agreement\label{fig:NA-good}}
\end{center}
\end{figure}
...
\label{section-affichage-des-automates-de-phrase}
...
\bigskip
\noindent Figure~\ref{fig-automate-première-phrase-Ivanhoe} shows the graph
obtained for the first sentence of \textit{Ivanhoe}. The words \verb+Ivanhoe+,
...
\caption{Automaton of the first sentence of \textit{Ivanhoe}\label{fig-automate-première-phrase-Ivanhoe}}
\end{center}
\end{figure}
...
figure~\ref{fig-automate-phrase-modifié}).
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=15cm]{resources/img/fig7-24.png}
\caption{Modified sentence automaton\label{fig-automate-phrase-modifié}}
\end{center}
\end{figure}
...
For further details, refer to section~\ref{section-présentation-polices-couleurs}.
...
text file in the window as shown on Figure \ref{fig-7-12}.
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=10cm]{resources/img/fig7-25.png}
\caption{Setting output file for linearization of the text automaton\label{fig-7-12}}
\end{center}
\end{figure}

\bigskip
\noindent If the automaton is not linear, an error message will give you the number of
the first sentence that contain ambiguity. Otherwise, the \verb+Fst2Unambig+ program
will build the output file according to the following rules:
\index{External programs!\verb+Fst2Unambig+}\index{\verb+Fst2Unambig+}
\begin{itemize}
...
\caption{Example of a linear text automaton\label{fig-7-13}}
\end{center}
\end{figure}

\bigskip
\noindent NOTE: correcting spaces in the output text can only be done manually. If the
original text is the one of the text automaton shown on Figure \ref{fig-7-13},
