...
is the  dictionary of inflected forms DELAF (DELA de formes Fléchies, DELA of
inflected forms) or DELACF (DELA de formes Composées Fléchies, DELA of compound
inflected forms) in the case of compound forms.\index{DELAF|(}
...
formes simples, simple forms DELA) or DELAC (DELA de formes composées, compound
forms DELA).
...
\label{section-format-des-DELAF}
...
\begin{itemize}
  ...
  designates a concrete object (see table~\ref{tab-codes-sémantiques}).
  ...
  table~\ref{tab-codes-flectionnels}) Since each character represents one
  information, it is not necessary to use the same character more than once. In
  this way, encoding the past participle using the code \verb+:PP+ would be
...
have different meanings, as it is the case for the French word \textit{poêle}
...
\label{section-format-des-DELAS}
...
section~\ref{section-flexion-automatique}.
...
\hline
\end{tabular}
\caption{Frequent grammatical codes\label{tab-codes-grammaticaux}}
\end{center}
\end{table}
...
\hline
\end{tabular}
\caption{Some semantic codes\label{tab-codes-sémantiques}}
\end{center}
\end{table}

\bigskip
\noindent NOTE: The  descriptions of tense in table~\ref{tab-codes-flectionnels} correspond
to French. Nontheless, the majority of these definitions can be found in other
languages (infinitive, present, past participle, etc.).
...
\hline
\end{tabular}
\caption{Common inflectional codes\label{tab-codes-flectionnels}}
\end{center}
\end{table}
...
\caption{Dictionary example\label{fig-exemple-dictionnaire}}
\end{center}
\end{figure}

\noindent Let's load the dictionary as in figure~\ref{fig-exemple-dictionnaire}.
Then, click on "Check Format..." in the
"DELA" menu. A window like in figure~\ref{fig-vérification-dictionnaire} is
opened. You must select the type of dictionary you want to check.
After checking the dictionary in Figure~\ref{fig-exemple-dictionnaire}, 
results are presented as shown in Figure~\ref{fig-résultat-vérification-dictionnaire}.
...
\caption{Checking a dictionary\label{fig-vérification-dictionnaire}}
\end{center}
\end{figure}
...
\caption{Results of checking\label{fig-résultat-vérification-dictionnaire}}
\end{center}
\end{figure}
...
chapter \ref{chap-programmes-externes}).
...
the characters  \verb+e+  and \verb+é+  is ignored if one wants to compare the
words \verb+manger+ et \verb+mangés+ because the contexts \verb+r+ and \verb+s+
allow to decide the order. The difference is only taken into account when the contexts are
identical, as they are when comparing \verb+pêche+ and \verb+pèche+.
...
difference between lower and upper case. Accents and the cédille character are
ignored as well.
...
\label{section-flexion-automatique}
\index{Automatic inflection}\index{Dictionaries!automatic inflection}
\subsection{Inflection of simple words}

As described in section~\ref{section-format-des-DELAS}, a line in a DELAS
...
window in figure~\ref{fig-configuration-flexion} allows you to specify the
directory in which inflectional grammars are found. By default, the subdirectory
\verb+Inflection+ of the directory for the current language is used. 

\bigskip
\begin{figure}[h]
\begin{center}
\includegraphics[width=8cm]{resources/img/fig3-5.png}
\caption{Configuration of automatic inflection\label{fig-configuration-flexion}}
\end{center}
\end{figure}
...
\caption{Inflectional grammar \texttt{N4}\label{fig-exemple-grammaire-de-flexion}}
\end{center}
\end{figure}

\bigskip
\noindent Figure~\ref{fig-exemple-grammaire-de-flexion} shows an example of an
...
code \verb+:mp+. Five operators are possible:
...
\begin{itemize}
  ...
  \verb+è+: \verb+peler+ $\rightarrow$ \verb+pèle+, \verb+acheter+ $\rightarrow$
  \verb+achète+, \verb+gérer+ $\rightarrow$ \verb+gère+, etc. Instead of
  describing an inflectional suffix for each verb (\verb+LLLLèle+, \verb+LLLLète+
  et \verb+LLLLère+), the \verb+R+ operator can be used to describe it in one
  way: \verb+LLLLèRR+.
  ...
  any. For instance the sequence \verb+LLUx+ applied to the word \verb+mangés+
  produces the inflected form \verb+mangex+, since \verb+U+ has turn the \verb+'é'+
  into a \verb+e+.
\end{itemize}
...
skeletons}\index{Consonant skeleton}. A lemma is made of consonants, and the
...
Figure~\ref{fig-exemple-automate-minimal}. \bigskip \begin{figure}[!h]
\begin{center}
\includegraphics[width=5cm]{resources/img/fig3-8.png}
\caption{Representation of a minimal automaton\label{fig-exemple-automate-minimal}}
\end{center}
\end{figure}
...
created. Figure~\ref{fig-resultat-compression} shows the result of the
compression of a dictionary of simple words. \bigskip
\begin{figure}[!h]
\begin{center}
\includegraphics[width=14cm]{resources/img/fig3-9.png}
\caption{Results of a compression\label{fig-resultat-compression}}
\end{center}
\end{figure}
...
\label{section-application-dictionnaires}
...
section~\ref{section-application-dictionnaires}).
...
in detail. Dictionary graphs will be described in section~\ref{graphes-dicos}.

\subsection{Priorities}
\label{section-priorités-dictionnaires}
...
\label{section-règles-application-des-transducteurs}
...
\label{graphes-dicos}\index{Dictionary graphs}
...
graph \verb$NPr+$ shown in figure~\ref{graphe-NPr}. The \verb$+$ in the graph
...
For more information about contexts,\index{Context} see section \ref{contextes}.

\begin{figure}[!h]
\begin{center}
\includegraphics[width=10.5cm]{resources/img/fig3-11.png}
\caption{Dictionary graph that tags unknown words beginning with an uppercase letter as proper names\label{graphe-NPr}}
\end{center}
\end{figure}
...
graph shown on Figure \ref{graphe-CR} use morphological filters to recognize
...
\caption{Dictionary graph of roman numerals\label{graphe-CR}}
\end{center}
\end{figure}
...
using the morphological mode (see section \ref{morphological_mode}). This
functionality will be helpful for agglutinative languages like Korean.
