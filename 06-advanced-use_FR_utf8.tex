...
\bigskip
\noindent Toutefois, vous pouvez mémoriser des choses avec des variables (voir section
...
Vous pouvez associer des variables aux motifs qui font références aux  dictionnaires morphologiques,
sauf \verb+<DIC>+. Il suffit de mettre en sortie de la boîte \verb+$xxx$+ où \verb+xxx+ est un nom
correct de variable. Ceci définit une  variable dénommée \verb+xxx+ qui représente l'entrée de
dictionaire reconnue avec votre motif.
Maintenant il est possible d'obtenir, la forme canonique et les informations sur l'entrée avec
\verb+$xxx.INFLECTED$+, \verb+$xxx.LEMMA$+ et \verb+$xxx.CODE$+, comme le montre la figure
\ref{fig-morpho5}.
Vous pouvez également utiliser les motifs suivants:

\begin{itemize}
\item \verb+$xxx.CODE.GRAM$+: fournit seulement le premier code grammatical,
	supposé être la catégorie grammaticale
  
\item \verb+$xxx.CODE.SEM$+: fournit tous les autre codes,
	séparés par des \verb$+$, s'il en existe
  
\item \verb+$xxx.CODE.FLEX$+: fournit tous les codes flexionnels
  séparés par des \verb$:$, s'il en existe

\item \verb+$xxx.CODE.ATTR=yyy$+ renvoi la valeur \verb+zzz+ de l'attribut \verb+yyy+ si un  code sémantique de la forme \verb+yyy=zzz+ existe.
...
\noindent Pour les grammaires appliquées avec \verb+LocateTfst+, vous disposez d'une possibilité
supplémentaire. 
Même sans être en mode morphologique, vous pouvez mémoriser dans une variable morphologique
l'information associée à une  étiquette lexicale contenue dans l'automate du texte.
Il vous suffit pour cela d'associer à la boîte une sortie de la forme \verb+$:abc$+ où \verb+abc+
est le nom de votre variable.
Vous pouvez ensuite l'utiliser comme variable morphologique habituelle.
...
\bigskip
\noindent Si le début ou la fin d’une variable sont mal définis (fin d’une variable avant son début,
...
\section{Variables de sorties}
\label{section-output-variables}
\index{Variables de sorties}
...
Remarquons que lorsque une variable de sortie est déclarée, les sorties ne sont pas émises, elles
sont seulement mémorisées dans les variables de sorties.
...
la variable \verb+A+. En outre, les variables de sorties mémorisent seulement des sorties
effectivement produites par votre grammaire. Ainsi, même si vous travaillez en mode MERGE, les
variables de sorties ne mémorisent jamais le texte d'entrée.
...
reconnaisance courante si la condition n'est pas vérifiée. Ceci se fait en insérant la  séquence
...
et le programme repart en arrière. Ceci fonctionne sur des variables normales, des variables de
sorties ou des variables morphologiques. De façon similaire, on peut vérifier qu'une variable n'est
...
\index{Traitement des erreurs sur les variable}
\bigskip
