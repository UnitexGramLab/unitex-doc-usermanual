...
\section{Seq2Grf}
\index{External programs!\verb+Seq2Grf+}
\index{\verb+Seq2Grf+}
\label{Seq2Grf}
...
\begin{itemize}
  \item \verb+-a ALPH+/\verb+--alphabet=ALPH+: the alphabet file to use;
  \item \verb+-o GRF+/\verb+--output=XXX+: output GRF file;
  \item \verb+--b+: beautify the produced graph;
  \item \verb+-j x+: number of jokers;
  \item \verb+-i x+: number of insertions;
  \item \verb+-r x+: number of replations;
  \item \verb+-d x+: number of deletions;
  \item \verb+-h+/+--help+: this help;
\end{itemize}
...
\noindent Constructs the sequences automaton : one single automaton that recognizes
	...
	The result files, named \verb+text.tfst+, \verb+text.tind+ and \verb+XXX+, the GRF file
	are stored is the text directory.
