...
When Arabic linguists read dictionaries in order to spot errors in dictionaries,
reading them in the Arabic script is simple and efficient. However, when they
create inflectional grammars (\ref{section-automatic-inflection}), parts of
Arabic words occur in the same box as morpho-syntactic annotation encoded
in the Latin alphabet. In this context, switching from right-to-left Arabic script
to left-to-right Latin alphabet is a real hassle, and Unitex offers facilities that allow
users to encode their inflectional grammars in the Buckwalter++
transliteration,\index{Buckwalter++}
a one-to-one mapping between a Unicode encoding of Arabic script and
Latin letters. The Buckwalter++ transliteration is defined by the table
of Fig.~\ref{buckwalter1} and \ref{buckwalter2} (cf.~\cite{neme2011},
Section~3.2).
Unitex provides tools to internally transliterate DELA and DELAF Arabic
dictionaries to and from Buckwalter++ encoding
(Fig.~\ref{Arabic-transliteration}).

\begin{figure}[!p]
\begin{center}
\includegraphics[height=19.4cm]{resources/img/buckwalter1.pdf}
\caption{Buckwalter++ transliteration\label{buckwalter1}}
\end{center}
\end{figure}

\begin{figure}[!p]
\begin{center}
\includegraphics[height=19.4cm]{resources/img/buckwalter2.pdf}
\caption{Buckwalter++ transliteration\label{buckwalter2}}
\end{center}
\end{figure}
...
Figure~\ref{fig-example-minimal-automaton}. \bigskip \begin{figure}[!ht]
\begin{center}
