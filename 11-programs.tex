...
\section{Creating log with UnitexToolLogger}\index{Creating log with UnitexToolLogger}
...
a second-order hidden Markov model. If the specified tagger data file contains tuples of raw tags,
the tagger could prune transitions only on grammatical codes (for example, \verb+under.A+ versus \verb+under.ADV+).
Else if it contains tuples of inflected tags, so the tagger could prune transitions on grammatical 
...
formed by tags and words. In the first data file, tags are raw tags (i.e. only grammatical codes). In the second data file,
tags are inflected tags (i.e. grammatical and inflectional codes). They are used by \verb+Tagger+ program in order to compute
...
\begin{itemize}
  ...
  \item \verb+-r/--raw+: indicates whether the program should produce only data file with raw tags;
  \item \verb+-i/--inflected+: indicates whether the program should produce only data file with inflected tags;
  ...
  \item \verb+-o XXX/--output=XXX+: pattern used to name output tagger data files \verb+XXX_data_raw.bin+
  and \verb+XXX_data_inflected.bin+ (default=filename of text corpus without extension);
\end{itemize}
