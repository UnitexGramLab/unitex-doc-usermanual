...
the tagger could prune transitions only on inflectional codes (for example, \verb+under.A+ versus \verb+under.ADV+).
Else if it contains tuples of compound form tags, so the tagger could prune transitions on inflectional codes 
and also semantic codes (for example, \verb+the.DET:s+ versus \verb+the.DET:p+). That's why the automata needs to be 
