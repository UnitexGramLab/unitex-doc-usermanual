...
\label{chap-programmes-externes}
...
automatically called by the interface. It is possible to see the commands that
have been executed by clicking on "Info>Console". It is also
possible to see the options of the different programs on
"Info>Help on commands".
\bigskip
...
parameters.
\index{Text!directory of}\index{External programs!\verb+Normalization+}\index{\verb+Normalization+}
...
of DELAS or DELAF type. The parameter \verb+dic+ corresponds to the name
...
\begin{itemize}
  ...
  the right of the occurrences. If the occurrence is shorter than this value, the
  concordance line is completed up to \verb+right+. If the occurrence is longer
...
\end{itemize}
\noindent For details on the sorting modes, see section \ref{section-affichage-des-résultats}.
...
\begin{itemize}
  ...
  \item \verb+-m TXT+/\verb+--merge=TXT+: indicates to the program that it is supposed to
    ...
    \verb+TXT+ (see section \ref{section-modification-du-texte}).
\end{itemize}
...
\verb+concord.html+ if the output mode was \verb+--html+ or \verb+--glossanet+,
and a text file with the name defined by the user of the program if the program has
constructed a modified version of the text.
\bigskip
...
that shows their differences (see section \ref{concordiff}, page \pageref{concordiff}).
...
\ref{section-application-dictionnaires}, page \pageref{section-application-dictionnaires}).
\index{File!\verb+.bin+}It is possible to give priorities to the dictionaries. For details see 
section \ref{section-priorités-dictionnaires}.
\bigskip
...
\begin{itemize}
  ...
  (see section \ref{graphes-dicos}, page \pageref{graphes-dicos});
...
\verb+Elag [OPTIONS] <txtauto>+

\bigskip
\noindent This program takes a \verb+.fst2+ text automaton \verb+<txtauto>+ and
applies to it ambiguity removal rules.\index{File!\verb+.fst2+}
...
\begin{itemize}
  ...
  \item \verb+-d DIR/--directory=DIR+: directory where ELAG rules are located.
\end{itemize}
...
\begin{itemize}
  ...
  \item \verb+-o OUT+/\verb+--output=OUT+: output file. By default, the output file name is 
        the same as \verb+RULES+, except for the extension that is \verb+.rul+;\index{File!\verb+.rul+}
  \item \verb+-d DIR+/\verb+--directory=DIR+: directory where ELAG rules are located.
\end{itemize}


\section{Evamb}\index{External programs!\verb+Evamb+}\index{\verb+Evamb+}
\verb+Evamb [OPTIONS] <txtauto>+

\bigskip
\noindent This program computes an average lexical ambiguity rate on the text
automaton \verb+<txtauto>+, or just on the sentence which number is specified by
...
\section{ExplodeFst2}\index{External programs!\verb+ExplodeFst2+}\index{\verb+ExplodeFst2+}
\verb+ExplodeFst2 [OPTIONS] <txtauto>+

\bigskip
\noindent This program computes and stores in \verb+OUT+ the \textit{developed} form of the
text automaton \verb+<txtauto>+.\index{Text automaton!developed}

\bigskip
\noindent \textbf{OPTIONS:}
\begin{itemize}
  \item \verb+-o OUT+/\verb+--output=OUT+: output file. By default, \verb+OUT+
  is made of \verb+<txtauto>+ with \verb+-exp+ before the extension like:
  \verb+foo-exp.fst2+.
\end{itemize}
...
\begin{itemize}
  ...
  \medskip
  \verb$il fait [très très]*$
  
  \verb$il fait très beau$
...
\section{Fst2Unambig}\index{External programs!\verb+Fst2Unambig+}\index{\verb+Fst2Unambig+}
\index{Text automaton!conversion into linear text}
\verb+Fst2Unambig [OPTIONS] <fst2>+

\bigskip
\noindent This programs takes a \verb$.fst2$ text automaton and produces an
equivalent text file if the automaton is linear (i.e. with no
ambiguity). See section \ref{linear-text}, page \pageref{linear-text}.
...
section \ref{section-compiler-une-grammaire}). The parameter \verb+graph+ 
...
\begin{itemize}
  ...
  use (see section \ref{repository-section}, page \pageref{repository-section}).
...
the same folder as \verb+graph+.


\section{ImplodeFst2}\index{External programs!\verb+ImplodeFst2+}\index{\verb+ImplodeFst2+}
\verb+ImplodeFst2 [OPTIONS] <txtauto>+

\bigskip
\noindent This program computes and stores in \verb$OUT$ the \textit{compact}
form of the text automaton \verb$<txtauto>$.\index{Text automaton!compact}
\bigskip
\noindent \textbf{OPTIONS:}
\begin{itemize}
  \item \verb+-o OUT+/\verb+--output=OUT+: output file. By default, \verb+OUT+
  is made of \verb+<txtauto>+ with \verb+-imp+ before the extension like:
  \verb+foo-imp.fst2+.
\end{itemize}
...
\section{MergeTextAutomaton}\index{External programs!\verb+MergeTextAutomaton+}\index{\verb+MergeTextAutomaton+}
\index{File!\verb+text.fst2.bck+}
\verb+MergeTextAutomaton <txtauto>+

\bigskip
\noindent \index{Text!automaton}\index{Reconstruction of the text automaton}This 
program reconstructs text automaton \verb+<txtauto>+ taking into account the
manual modifications. If the program finds a file \verb+sentenceN.grf+ in the
same directory as \verb+<txtauto>+, it replaces the automaton of sentence
\verb+N+ with the one represented by \verb+sentenceN.grf+. The \verb+<txtauto>+
file is replaced by the new text automaton. The old text automaton is backed up
in a file called \verb+text.fst2.bck+.
...
containing simple (see section \ref{section-format-des-DELAS}) or compound word
...
\noindent \textbf{OPTIONS:}
\begin{itemize}
  \item \verb+-n+/\verb+--no_carridge_return+: every separator sequence will be turned into a single space;
  
  \item \verb+-r XXX+/\verb+--replacement_rules=XXX+: specifies the
  normalization rule file to be used. See section \ref{norm-file} for details about the format of
...
\verb+ForbiddenWords.txt+ (see section \ref{forbidden-words}) and stored in thesame directory than \verb+BIN+.
...
\section{TagsetNormFst2}\index{External programs!\verb+TagsetNormFst2+}\index{\verb+TagsetNormFst2+}
\verb+TagsetNormFst2 [OPTIONS] <txtauto>+

\bigskip
\noindent This program normalizes the specified \verb+.fst2+ text automaton according
...
and \verb+{rouge,.A:ms}+. The text automaton is modified.
...
\begin{itemize}
  ...
  the best paths (see section \ref{section-conservation-des-meilleurs-chemins}) 
...
\begin{itemize}
  ...
  rule file to be used (see section \ref{norm-file});
