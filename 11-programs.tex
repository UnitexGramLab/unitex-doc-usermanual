...
to UnitexTool.exe under Windows). Then creates an UTF8 text file named
unitex_logging_parameters.txt. On this text file, put at the first line the path of
...
Unitex uses \ref{unicode-encoding}Unicode for text file. All program which read or write
...
\bigskip
\noindent This program check if a fst2 graph has no error for locate.
...
\begin{itemize}
  ...
  \item \verb+-a+/\verb+--append+: to open the output file in append mode;
  \item \verb+-s+/\verb+--statistics+: display statistics about fst2 file.
\end{itemize}
...
If UnitexToolLogger is used like UnitexTool (with another parameter than RunLog), and
a file named unitex_logging_parameters_count.txt (in current directory) contain a path, 
