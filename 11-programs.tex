...
"Info>Help on commands" (see Figure \ref{fig-help}). Note that that all Unitex
...
\begin{itemize}
  ...
  \item \verb+-a ALPH+/\verb+--alphabet=ALPH+: specifies the alphabet file to
        use. This information is used to know wether a word is a simple one
        (only made of letters) or not. If not specified, a default default
        definition of letters is used (see \verb$Unicode.cpp$ source file).
...
\begin{itemize}
  ...
  \item \verb+-j TABLE+/\verb+--jamo=TABLE+: specifies the Jamo conversion
  table to use for Korean.
\end{itemize}
...
\section{ImplodeTfst}\index{External
programs!\verb+ImplodeTfst+}\index{\verb+ImplodeTfst+} \verb+ImplodeTfst [OPTIONS] <tfst>+
...
\begin{itemize}
  ...
  \item \verb+-j TABLE+/\verb+--jamo=TABLE+: Jamo conversion table to use for
  Korean.
\end{itemize}
...
\begin{itemize}
  ...
  \item \verb+-j TABLE+/\verb+--jamo=TABLE+: Jamo conversion table to use for
  Korean;
  \item \verb+-f FST2+/\verb+--fst2=FST2+: Jamo $\rightarrow$ Hangul transducer
  to use for Korean;
...
programs!\verb+UnitexTool+}\index{\verb+UnitexTool+}\index{\verb+Scripting Unitex programs+}
