...
\begin{itemize}
  ...
  \item Version améliorée de \verb$CasSys$~: nouveaux fichiers \verb$csc$, suppression du répertoire \verb$Share$, ouverture de cascade possible aussi avec le menu \verb$FSGraph$, application de graphe jusqu'au point fixe, graphes génériques, redénormalisation du dernier fichier
  ...
  \item Introduction de \verb$RunScript$, un outil qui exécute des scripts installés par \verb$InstallLingResourcePackage$ (non encore documenté). Avec ces deux outils, on peut mettre au point des opérations Unitex dans un environnement et les déployer dans un autre.
  \item Introduction de l'option "match word boundaries" dans l'algorithme de recherche par intersection d'automates (\ref{section-locate-tfst}), de sorte que, par exemple, \textit{enlever} and \textit{en lever} ne matchent pas (ou matchent, si on le désire). Non encore documenté.
