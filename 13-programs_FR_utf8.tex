...
\index{Texte!répertoire d}\index{Programmes externes!\verb+Normalize+}\index{\verb+Normalize+}
...
\index{Genération du dictionnaire des mots composés Coréen}\index{Dictionnaire des mots composés
Coréen}
...
\index{Vérification du format du dictionnaire}\index{Dictionnaire!vérification}
...
\index{Dictionnaires!DELAF}\index{Dictionnaires!compression}
...
\begin{itemize}
  ...
  \item \verb+-l X+/\verb+--left=X+: nombre de caractères à gauche des occurrences (par défaut=0).
  	  ...
  	  diacritiques.\index{Contextes!concordance}
...
\bigskip
\noindent \textbf{Options de tri:}\index{Tri!concordances}
\begin{itemize}
...
\item \verb+-m DICS+/\verb+--morpho=DICS+: ce paramètre optionnel indique
	quels dictionnaires morphologiques doivent être utilisés, si exigés par des dictionnaires
	\verb+.fst2+. \verb+DICS+ représente une liste de fichiers \verb+.bin+ (avec leur nom
...
\item \verb+-s+/\verb+--semitic+: indique à \verb$Dico$ qu'il travaille sur une langue sémitique
	(exigé  si \verb$Dico$ doit compresser un dictionnaire morphologique);
...
\index{Graphe!approximation par transducteur à états finis}
\index{Approximation d'une grammaire par un transducteur à états finis}
...
\item \verb+-n+/\verb+--no_loop_check+: désactive la vérification d'erreurs (par défaut);
	\index{Détection d'erreur dans les graphes}\index{Graphes!détection d'erreur }\index{Erreurs
	dans les graphes}
...
\index{Graphes!compilation}\index{Compilation de graphes}
...
\begin{itemize}
  \item \verb+-y+/\verb+--loop_check+: active la vérification d'erreurs (
		détection de boucles);
  \item \verb+-n+/\verb+--no_loop_check+: désactive la vérification d'erreurs (par défaut);
  	  \index{Détection d'erreurs dans les graphes}\index{Graphe!détection d'erreurs}\index{Erreurs dans les
  	  graphes}
...
\bigskip
\noindent \index{Dictionnaires!flexion automatique}\index{Flexion automatique}Ce programme effectue
...
\index{Mots!composés!libres en norvégien}
\index{Mots!composés!libres en allemand}
\index{Mots!composés!libres en russe}
\index{Mots!composés!libres en néerlandais}
\index{Analyse des mots composés libres en nor-végien}
\index{Analyse des mots composés libres en allemand}
\index{Analyse des mots composés libres en russe}
\index{Analyse des mots composés libres en néerlandais}
...
\index{Compression des dictionnaires}\index{Dictionnaires!compression}
\index{Clitiques!normalisation}\index{Normalisation!clitiques en Portuguais}
\index{Portuguais!normalisation des clitiques}
...
\bigskip
\noindent \index{Expressions régulières}
...
\bigskip
\noindent \index{Lexique-grammare!tables}\index{Graphe!principal}
...
\index{Automate!texte}\index{Texte!automate}
...
\index{Texte automate!conversion en texte linéaire}
...
\bigskip
\noindent \index{Lexicon-grammar!tables}\index{Graphe!principal}Ce programme génère automatiquement deux
...
\item \verb+-c/--cat+: indique que le programme ne doit produire que les fichiers de données avec
  ...
  \item \verb+-s/--semitic+: indique que l'algoritme de compression sémitique dot être utilisé;
\end{itemize}
...
\section{UnitexTool}
\index{Programmes externes!\verb+UnitexTool+}\index{\verb+UnitexTool+}\index{Scripting Unitex
programs}
