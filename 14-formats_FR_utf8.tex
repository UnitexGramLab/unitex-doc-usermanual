...
\index{Fichier!formats}\index{Formats de fichiers}
...
\index{Fichier!\verb+.txt+}\index{\verb+{S}+}\index{Etiquettes!lexicales}
\index{Séparateur de phrase}
\label{section-texts}
...
étiquettes lexicales (\verb+{aujourd'hui,.ADV}+)\index{Etiquetteslexicales}.
...
hypertextes. La référence associée à chacun de ces liens est de la forme \verb+<a href="X Y Z">+.
\verb+X+ et \verb+Y+ représentent les positions de début et de fin de l’occurrence en caractères
...
(\verb+{aujourd'hui,.ADV}+)\index{Etiquette!lexicales} et les phrases sont délimitées par
...
Lorsque l’utilisateur modifie ses préférences pour une langue donnée, celles-ci sont sauvergardées
...
\bigskip
\noindent Le paramètre \verb+MORPHOLOGICAL DICTIONARY+ indique la liste des dictionnaires morphologiques à utiliser séparés par des points virgules.
...
\subsection{Fichier user.cfg et .unitex.cfg}
\index{Fichier!\verb+.cfg+}
Sous Linux, Unitex considère que le répertoire personnel de l’utilisateur se nomme \verb+unitex+ et
...
le chemin vers votre répertoire Unitex privé. Ce fichier est un fichier UTF8.

\bigskip
\noindent Sous Windows, il n’est pas toujours possible d’associer un répertoire par défaut à un
utilisateur. Pour remédier à cela, Unitex crée pour chaque utilisateur un fichier \verb+.cfg+
contenant le chemin de son répertoire personnel. Ce fichier est sauvegardé sous le nom
\verb+(user login).cfg+ dans le sous-répertoire du système \verb+Unitex/Users+.
...
\index{Fichier de log} \label{section-log-file}
